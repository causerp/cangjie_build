...
This guide is designed to help users: set up the Cangjie SDK build environment in Linux systems and build a complete `linux-ohos` Cangjie SDK with all components.
...
export SDK_NAME=linux-x64-ohos # or linux-aarch64-ohos
```
...
cd $WORKSPACE/software;
# Copy stdx directory
cp -R $WORKSPACE/cangjie_stdx/target/linux_ohos_*_cjnative .;
...
chmod -R 750 linux_ohos_*_cjnative;
zip -qr cangjie-stdx-ohos-x64-${CANGJIE_VERSION}.${STDX_VERSION}.zip linux_ohos_x86_64_cjnative;
zip -qr cangjie-stdx-ohos-aarch64-${CANGJIE_VERSION}.${STDX_VERSION}.zip linux_ohos_aarch64_cjnative;
chmod 550 cangjie-stdx-*-${CANGJIE_VERSION}.${STDX_VERSION}.zip;
```
...
# - cangjie-stdx-ohos-x64-${CANGJIE_VERSION}.${STDX_VERSION}.zip
# - cangjie-stdx-ohos-aarch64-${CANGJIE_VERSION}.${STDX_VERSION}.zip
```
