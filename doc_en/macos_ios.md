...
This guide is designed to help users: set up the Cangjie SDK build environment in macOS systems and build a complete `mac-ios` Cangjie SDK with all components.
...
- **CPU Architecture**: **Must** set corresponding environment variables based on your computer's chip architecture (x86_64 or aarch64). Note that `mac x86_64` cannot build runtime and libraries for `mac aarch64`.
- **Xcode**: This guide uses `Xcode 15.4` as an example
...
export SDK_NAME=mac-aarch64-ios # or mac-x64-ios
